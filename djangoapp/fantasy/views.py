...
class LeagueHome(ListView):
    ...
    def get_queryset(self):
        return sorted(
            Player.objects.filter(league__name__icontains=self.kwargs["league_name"]),
...
class Rounds(ListView):
    ...
    def get_queryset(self):
        return Round.objects.filter(league__name__icontains=self.kwargs["league_name"])
...
class RoundDetail(ListView):
    ...
    def get_queryset(self):
        ...
        qs = sorted(
            Player.objects.filter(league__name__icontains=self.kwargs["league_name"]),
            key=lambda a: a.points(round=round),
            reverse=True,
        )
        return qs
...
class PlayerPointsView(AdminCheckMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["rule"] = LeagueRule.objects.get(
            number=self.kwargs["rule_number"], league=league
        )
    ...
    def get_form(self):
        league = League.objects.get(name__icontains=self.kwargs["league_name"])
        rule = LeagueRule.objects.get(league=league, number=self.kwargs["rule_number"])
        round = Round.objects.get(league=league, number=self.kwargs["round_number"])
        initial = {"round": round, "rule": rule, "points": 0}
        ...
        if self.request.method == "POST":
            ...
            formset = self.formset_class(
                rule=rule, form_kwargs=kwargs, empty_initial=initial
            )
            for form in formset:
    ...
    def get_form_kwargs(self):
        league = League.objects.get(name__icontains=self.kwargs["league_name"])
        round = Round.objects.filter(number=self.kwargs["round_number"], league=league)
        rule = LeagueRule.objects.get(league=league, number=self.kwargs["rule_number"])
    ...
    def get_success_url(self):
        ...
        next_rule = (
            LeagueRule.objects.filter(
                league__name=league_name, number__gt=current_rule_number
            )
            .order_by("id")
            .first()
        )
